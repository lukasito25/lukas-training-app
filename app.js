// Lucide React icons (simplified inline SVG definitions)
const CheckCircle2 = ({ className, ...props }) => (
...
const RotateCcw = ({ className, ...props }) => (
  <svg className={className} {...props} fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M1 4v6h6m16 10v-6h-6m-7-1a7 7 0 1114 0"></path>
  </svg>
);

const Trophy = ({ className, ...props }) => (
  <svg className={className} {...props} fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M5 8h10l1.5 9H3.5L5 8zm5-6v2m0 16v-2"></path>
  </svg>
);
...
const Play = ({ className, ...props }) => (
  <svg className={className} {...props} fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
    <polygon points="5,3 19,12 5,21" strokeWidth="2"></polygon>
  </svg>
);
...
const FileText = ({ className, ...props }) => (
  <svg className={className} {...props} fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
    ...
    <polyline points="14,2 14,8 20,8" strokeWidth="2"></polyline>
    <line x1="16" y1="13" x2="8" y2="13" strokeWidth="2"></line>
    <line x1="16" y1="17" x2="8" y2="17" strokeWidth="2"></line>
    <polyline points="10,9 9,9 8,9" strokeWidth="2"></polyline>
  </svg>
);
...
const Save = ({ className, ...props }) => (
  <svg className={className} {...props} fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
    ...
    <polyline points="17,21 17,13 7,13 7,21" strokeWidth="2"></polyline>
    <polyline points="7,3 7,8 15,8" strokeWidth="2"></polyline>
  </svg>
);
...
const TrainingProgram = () => {
  ...
  const [exerciseWeights, setExerciseWeights] = React.useState({});
  ...
  const updateWeight = (day, exerciseIndex, change) => {
  ...
  const toggleNutritionGoal = (day, goalIndex) => {
  ...
  const saveSession = (dayName) => {
    const currentPhase = getPhase(currentWeek);
    const exercises = workouts[dayName].exercises[currentPhase];

    // Get completed exercises for this day
    const dayExercises = exercises.map((exercise, index) => ({
      name: exercise.name,
      sets: exercise.sets,
    ...
    const session = {
      ...
      exercisesCompleted: dayExercises.filter(e => e.completed).length,
      totalExercises: dayExercises.length,
      nutritionCompleted: dayNutrition.filter(n => n.completed).length,
      totalNutrition: dayNutrition.length
    };
    ...
    // Show confirmation
    alert(`✅ Session saved!\n${dayName}\nExercises: ${session.exercisesCompleted}/${session.totalExercises}\nNutrition: ${session.nutritionCompleted}/${session.totalNutrition}`);
  };
            ...
            <div key={dayName} className="bg-white rounded-xl shadow-sm overflow-hidden border">
              ...
              <div className="divide-y divide-gray-100">
                {exercises.map((exercise, index) => {
                  ...
                  const demos = getDemoUrl(exercise.demo);
                ...
                <div className="grid grid-cols-2 gap-2">
                  {dailyNutritionGoals.map((goal, index) => {
                    ...
                    return (
                      <div
                        key={index}
                        className={`flex items-center gap-2 p-2 bg-white rounded-lg border-l-4 ${categoryColors[goal.category]} ${
                ...
                <div className="space-y-4">
                  {completedSessions.map((session) => (
                    <div key={session.id} className="bg-gray-50 rounded-lg p-4 border">
                      ...
                      <div className="flex items-center justify-between mb-3">
                        <div>
                          <h3 className="font-bold text-gray-800">{session.dayName}</h3>
                          <p className="text-sm text-gray-600">
                            {session.date} at {session.time} • Week {session.week} ({session.phase} phase)
                          </p>
                        </div>
                        <div className="text-right">
                      ...
                      <div className="grid md:grid-cols-2 gap-4">
                        ...
                        <div>
                          ...
                          <div className="space-y-1 max-h-32 overflow-y-auto">
                            {session.nutrition.map((nutrition, index) => (
...
root.render(React.createElement(TrainingProgram));